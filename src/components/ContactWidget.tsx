...
export default function ContactWidget({ onOpenContact }: ContactWidgetProps) {
  const [isHovering, setIsHovering] = useState(false);

  // Get SNS links from config
  const emailLink =
    websites.favorites.sites.find((site) => site.id === "my-email")?.link ||
    "mailto:kang3171611@naver.com";
  const instagramLink =
    websites.favorites.sites.find((site) => site.id === "my-instagram")?.link ||
    "https://instagram.com/donghyeon179";
  const githubLink =
    websites.favorites.sites.find((site) => site.id === "my-github")?.link ||
    "https://github.com/kang1027";
  const xLink =
    websites.favorites.sites.find((site) => site.id === "my-x")?.link ||
    "https://x.com/mentis1027";
    ...
    >
      ...
      >
        ...
        <div className="absolute top-4 left-6 z-10">
          ...
          <p className="text-white/60 text-xs mt-0.5">메세지 보내기</p>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center pt-8">
          <div className="flex items-center gap-8">
            ...
            <div className="flex gap-4">
              {/* Email */}
              <div
                onClick={(e) => handleSocialClick(emailLink, e)}
              ...
              {/* Instagram */}
              <div
                onClick={(e) => handleSocialClick(instagramLink, e)}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 transition-all duration-300 cursor-pointer hover:scale-105 ${
                  isHovering ? "bg-white/10 border-white/20" : ""
                }`}
              >
              ...
              {/* GitHub */}
              <div
                onClick={(e) => handleSocialClick(githubLink, e)}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 transition-all duration-300 cursor-pointer hover:scale-105 ${
                  isHovering ? "bg-white/10 border-white/20" : ""
                }`}
              >
              ...
              {/* Twitter/X */}
              <div
                onClick={(e) => handleSocialClick(xLink, e)}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 transition-all duration-300 cursor-pointer hover:scale-105 ${
                  isHovering ? "bg-white/10 border-white/20" : ""
                }`}
              >
